...
class CloudStorageViewController: CloudStorageDisplayViewController {
    ...
    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        var actions = UploadType.allCases.map { type -> Action in
            Action(title: type.title, image: UIImage.fromPlugin(named: type.imageName), style: .default) { _ in
    ...
    lazy var addButton: UIButton = {
        ...
        addButton.setImage(UIImage.fromPlugin(named: "storage_add"), for: .normal)
        var actions = UploadType.allCases.map { type -> Action in
            Action(title: type.title, image: UIImage.fromPlugin(named: type.imageName), style: .default) { _ in
                UploadUtility.shared.start(uploadType: type,
                                           fromViewController: self,
    ...
    lazy var backItem: UIButton = {
        ...
        backItem.setImage(UIImage.fromPlugin(named: "arrowLeft")?.tintColor(.color(type: .text)), for: .normal)
        ...
        stack.setTraitRelatedBlock { tk in
            ...
            for (index, v) in tk.arrangedSubviews.enumerated() {
                ...
                if #available(iOS 13.0, *) {
                    btn.setImage(UIImage.fromPlugin(named: imageNames[index])?.tintColor(
                        .color(type: .text).resolvedColor(with: tk.traitCollection)
                    ), for: .normal)
                } else {
                    // Fallback for iOS 12 (no traitCollection or dynamic colors)
                    let tintColor = UIColor.color(type: .text)
                    btn.setImage(UIImage.fromPlugin(named: imageNames[index])?.tintColor(tintColor), for: .normal)
                }
            }
        }
        return stack
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if ConvertService.isFileConvertible(withFileURL: item.urlOrEmpty),
           item.meta.whiteConverteInfo == nil {
            print("file should convert without convert info \(item)")
...
extension CloudStorageViewController: UploadUtilityDelegate {
    @objc func presentTask() {
        if let presentedViewController {
            print("prevent present taskViewController for \(presentedViewController)")
            return
        }
