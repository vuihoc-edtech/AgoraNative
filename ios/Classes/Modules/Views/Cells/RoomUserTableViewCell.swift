...
class RoomUserTableViewCell: UITableViewCell {
    ...
    lazy var raiseHandButton: UIButton = {
        ...
        raiseHandButton.setTraitRelatedBlock { btn in
            raiseHandButton.setImage(UIImage.fromPlugin(named: "raisehand_small_icon")?.tintColor(.color(type: .primary)), for: .normal)
            raiseHandButton.setImage(UIImage.fromPlugin(named: "raisehand_small_icon")?.tintColor(UIColor.color(light: .grey3, dark: .grey6)), for: .disabled)
        }
    ...
    func updateCamera(on: Bool, enable: Bool) {
        let image = UIImage.fromPlugin(named: on ? "camera_on" : "camera_off")
    ...
    func updateMic(on: Bool, enable: Bool) {
        let image = UIImage.fromPlugin(named: on ? "mic_on" : "mic_off")
        let btnColor: UIColor
        switch (on, enable) {
        case (let isOn, true):
            btnColor = isOn ? .color(type: .primary) : .color(type: .danger)
