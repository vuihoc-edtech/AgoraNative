...
//            }
        ...
        itemsShouldPolling.forEach {
            ...
            case .projector:
                self.taskProgressPolling.insertProjectorPollingTask(withTaskUUID: taskUUID,
                    ...
                    print("task projector \(taskUUID) progress \(progress)")
            ...
            case .white:
                self.taskProgressPolling.insertV5PollingTask(withTaskUUID: taskUUID,
                    ...
                    print("task v5 \(taskUUID) progress \(progress)")
                } result: { [weak self] _, info, error in
                    if let error {
                        self?.toast(error.localizedDescription)
                        return
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.iconImage.image = UIImage.fromPlugin(named: item.fileType.iconImageName)
