...
class WhiteboardStyleViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func setupViews() {
        navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage.fromPlugin(named: "close-bold"), style: .plain, target: self, action: #selector(onClose))
...
class WhiteboardStyleCell: UICollectionViewCell {
    func update(style: WhiteboardStyle, selected: Bool) {
        ...
        selectImageView.image = UIImage.fromPlugin(named: selected ? "checklist_selected" : "checklist_normal")
