...
class UploadTasksViewController: UIViewController {
    ...
    func config(cell: UploadTaskTableViewCell, task: (UploadTask, BehaviorRelay<UploadStatus>), indexPath: IndexPath) {
        ...
        cell.iconImageView.image = UIImage.fromPlugin(named: fileType.iconImageName)
        ...
        task.1
            ...
            .subscribe(with: cell, onNext: { weakCell, status in
                ...
                weakCell.operationButton.setImage(UIImage.fromPlugin(named: status.statusOperationImageName)?.withRenderingMode(.alwaysOriginal), for: .normal)
    ...
    lazy var tableView: UITableView = {
        ...
        view.register(.init(nibName: String(describing: UploadTaskTableViewCell.self), bundle: AgoraNativePlugin.pluginBundle),
    ...
    lazy var topView: UIView = {
        ...
        closeButton.setImage(UIImage.fromPlugin(named: "upload_cancel"), for: .normal)
...
extension UploadTasksViewController: DZNEmptyDataSetSource {
    ...
    func image(forEmptyDataSet scrollView: UIScrollView!) -> UIImage! {
        UIImage(named: "cloud_empty", in: AgoraNativePlugin.pluginBundle, compatibleWith: traitCollection)
    }
}
