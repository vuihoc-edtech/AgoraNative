...
class ApiProvider: NSObject {
                             ...
                             responseDataHandler: ResponseDataHandler) -> Observable<T.Response>
    {
        ...
        return generator
            ...
            .do(onNext: {
                ...
                print("start rx \(String(describing: reqId)) \($0)")
            }
            )
            .flatMap {
                self.session.rx.data(request: $0)
            }
            .do(onNext: { print("raw rx \(String(describing: reqId)) \(String(data: $0, encoding: .utf8) ?? "")") })
            .flatMap {
                responseDataHandler.processObservableResponseData($0, decoder: api.decoder, forResponseType: T.Response.self)
            }
            .do(
                onNext: { obj in print("finish rx \(String(describing: reqId)) \(obj)") },
                onError: { error in print("finish error rx \(String(describing: reqId)) \(error)") }
            )
                             ...
                             completionHandler: @escaping (Result<T.Response, Error>) -> Void) -> URLSessionDataTask?
    {
        do {
            ...
            print("start \(reqId) \(req)")
            let task = session.dataTask(with: req) { data, response, error in
                if let data {
                    print("raw data \(reqId) \(String(data: data, encoding: .utf8) ?? "")")
                } else if let error {
                    print("finish error \(reqId) \(error)")
                }
